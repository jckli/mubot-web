...
export default function DashboardManager({ scope, state, mutateCards, mutateState, selectedManga, deletingManga, onCloseGroup, onCloseDelete }: {
  ...
  return <>
    ...
    <DashboardSheet open={mode === "add"} title="Add manga" onClose={() => setMode(null)}><input autoFocus value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search MangaUpdates" className="w-full rounded-lg border border-border/60 bg-secondary/40 px-3 py-2 outline-none ring-primary focus:ring-1" />
    ...
    <GroupSheet scope={scope} manga={selectedManga} onClose={onCloseGroup} mutateCards={mutateCards} />
...
export function GroupSheet({ scope, manga, onClose, mutateCards }: { scope: string; manga: MangaCard | null; onClose: () => void; mutateCards: KeyedMutator<MangaCard[]> }) {
  const [error, setError] = useState("");
