...
import { ArrowRight, LayoutDashboard, MessageCircle, Github, Activity } from 'lucide-react';
...
export default function HomeClient({ authenticated }: { authenticated: boolean }) {
    ...
    <main className="flex-1 flex flex-col p-4">
      ...
      <div className="flex-1 flex flex-col items-center justify-center space-y-6 text-center max-w-xl mx-auto w-full pt-8">
        ...
        <div className="flex flex-col sm:flex-row items-center gap-3">
          <PillButton
            href={authenticated ? "/dashboard" : "/api/auth/discord/start"}
            isExternal={false}
          ...
          >
            {authenticated ? <LayoutDashboard className="h-4 w-4" /> : <DiscordIcon className="h-4 w-4" />}
            <span>{authenticated ? "Open Dashboard" : "Login with Discord"}</span>
          </PillButton>
